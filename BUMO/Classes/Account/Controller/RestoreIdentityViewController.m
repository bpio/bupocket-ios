...
{
    [super viewDidLoad];
    self.navigationItem.title = @"恢复身份";
    [self setupView];
}
- (void)setupView
{
    ...
    _purseName = [UITextField textFieldWithplaceholder:@"请输入新钱包名" margin:30 height:35 font:15];
    ...
    _pursePassword = [self setupPWPlaceholder:@"请设置密码"];
    [self.view addSubview:_pursePassword];
    
    _confirmPassword = [self setupPWPlaceholder:@"请再次确认密码"];
    [self.view addSubview:_confirmPassword];
    
    _createIdentity = [UIButton createButtonWithTitle:@"创建" TextFont:18 TextColor:[UIColor whiteColor] Target:self Selector:@selector(createAction)];
...
- (PlaceholderTextView *)memorizingWords
{
    if (!_memorizingWords) {
        ...
        _memorizingWords.textColor = [UIColor colorWithHexString:@"666666"];
        _memorizingWords.placeholderColor = [UIColor colorWithHexString:@"B2B2B2"];
        _memorizingWords.placeholder = @"输入助记词，用空格分割";
        ...
        _memorizingWords.backgroundColor = [UIColor colorWithHexString:@"F5F5F5"];
    }
...
- (void)createAction
{
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否确认更新密码及导入钱包？" preferredStyle:UIAlertControllerStyleAlert];
