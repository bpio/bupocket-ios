...
- (void)confirmAction
{
    ...
    if (random) {
        [[HTTPManager shareManager] setAccountDataWithRandom:random password:self.PWNew.text identityName:[[AccountTool shareTool] account].identityName typeTitle:self.navigationItem.title success:^(id responseObject) {
