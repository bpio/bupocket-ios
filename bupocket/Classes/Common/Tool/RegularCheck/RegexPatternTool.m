...
#import "RegexPatternTool.h"

@implementation RegexPatternTool
...
//    "(^0(\\.[0-9]{0," + (decimals - 1) + "}[1-9])?$)|(^[1-9][0-9]{0," + (18 - decimals) + "}(\\.[0-9]{0," + (decimals - 1) + "}[1-9])?$)"
    NSString * regex = [NSString stringWithFormat:@"(^0(\\.[0-9]{0,%zd})?$)|(^[1-9][0-9]{0,%zd}(\\.[0-9]{0,%zd})?$)", decimals, 18 - decimals, decimals];
    if (decimals == 0) {
        regex = [NSString stringWithFormat:@"(^0?$)|(^[1-9][0-9]{0,18}?$)"];
    }
...
// 验证字符串是否为只含有汉字、数字、字母、下划线，下划线位置不限：
- (BOOL)validateUserName:(NSString *)targetString
{
...
//    NSString *userNameRegex = @"^[A-Za-z0-9]{4,20}+$";
    NSString *userNameRegex = @"^[a-zA-Z0-9_\u4e00-\u9fa5]{1,20}+$";
