...
- (UICollectionViewFlowLayout *)flowLayout
{
    if (!_flowLayout) {
        ...
        CGFloat h = self.height / count;
        _flowLayout.itemSize = CGSizeMake(DEVICE_WIDTH, h);
    }
    return _flowLayout;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.activityAwardsModel = self.dataSourceArray[indexPath.row];
//    cell.addressLabel.text = self.dataSourceArray[indexPath.row];
//
//    // 查看图片轮播demo使用
//    cell..image = [UIImage imageNamed:[NSString stringWithFormat:@"0%zd.jpg", indexPath.row]];
