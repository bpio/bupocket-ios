...
- (UILabel *)votesObtained
{
    if (!_votesObtained) {
        ...
        _votesObtained.textColor = COLOR_B2;
    }
...
- (UILabel *)numberOfVotes
{
    if (!_numberOfVotes) {
        ...
        _numberOfVotes.textColor = COLOR_B2;
    }
...
//    CGFloat nodeTypeW = [Encapsulation rectWithText:self.nodeType.text font:self.nodeType.font textHeight:ScreenScale(16)].size.width + Margin_10;
    ...
    self.votesObtained.attributedText = [Encapsulation attrWithString:[NSString stringWithFormat:@"%@ %@", vote, [NSString stringAmountSplitWith:nodePlanModel.nodeVote]] preFont:FONT(12) preColor:COLOR_B2 index:vote.length sufFont:FONT(14) sufColor:COLOR_6 lineSpacing:0];
    NSString * myVote = Localized(@"My votes");
    if ([nodePlanModel.myVoteCount longLongValue] < 2) {
        myVote = Localized(@"My vote");
    }
    self.numberOfVotes.attributedText = [Encapsulation attrWithString:[NSString stringWithFormat:@"%@ %@", myVote, [NSString stringAmountSplitWith:nodePlanModel.myVoteCount]] preFont:FONT(12) preColor:COLOR_B2 index:myVote.length sufFont:FONT(14) sufColor:COLOR_6 lineSpacing:0];
